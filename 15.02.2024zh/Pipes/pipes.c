...
/*   Updated: 2024/02/15 13:59:21 by zhedlund         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 */
void handle_child_process(t_cmd *cmd, int fd_pipe[], pid_t pid, t_env **head, t_info **info)
{
    close(fd_pipe[1]);
...
/* cmd: pointer to the command struct
	...
	note: the function is called by: handle_pipe_cmd()
 */
void handle_parent_process(t_cmd *cmd, int fd_pipe[], pid_t pid, t_env **head, t_info **info)
{
    close(fd_pipe[0]);
...
void handle_pipe_cmd(t_pipe *pipe_cmd, t_env **head, t_info **info)
{
	...
	if (pid == 0)
        	...
        	handle_parent_process((t_cmd *)pipe_cmd, fd_pipe, pid, head, info);
}
