...
void parse_tokens(t_exec *exec_cmd, t_cmd **cmd, char **position_ptr, char *end_str, t_info **info, t_env **head)
{
	...
	while (!check_next_token(position_ptr, end_str, "|"))
	{
		...
		if (token_type != 'q')
		{
			...
			expand_env(&exec_cmd->argv[args], (*info)->exitstatus, head); // Expand environment variables
    		expanded_token = expand_env_in_str(exec_cmd->argv[args], info); // Expand environment variables within double-quoted strings
